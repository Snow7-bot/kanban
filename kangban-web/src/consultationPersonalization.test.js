...
test('AI consultation switches authorized family members into isolated sessions', async () => {
  ...
  assert.match(page, /familyApi\.getFamilyMembers\(\)/);
  assert.match(page, /切换至\$\{nextName\}的独立问诊/);
  assert.match(page, /pendingSummaryMemberRef/);
  assert.match(page, /appendPatientSummary\(sid\)/);
  assert.match(page, /getChatSessions\(selectedMemberId\)/);
  assert.match(page, /contextVersion !== 'family-agent-v2'/);
  assert.match(page, /hasPersonalizedContext\(session, selectedMemberId\)/);
  assert.match(page, /Number\(context\.selectedMemberId\) === Number\(selectedMemberId\)/);
  assert.match(page, /memberId: selectedMemberId/);
  assert.match(page, /当前分析仅使用\{patientDisplay\.name\}的授权健康数据/);
  assert.match(api, /params: \{ memberId \}/);
  assert.match(api, /sessions\/\$\{sessionId\}\/summary/);
  assert.match(contracts, /memberId: member\?\.id \?\? null/);
});
